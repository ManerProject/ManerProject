...
echo "<br>".$imie." ".$nazwisko."<br>";
$sumat=0;
$poms=0;
if ($result89 = $wynik->query("SELECT `oceny` FROM `ocenys2` where idlogin='$uczenid'"))
{
    while($s=$result89->fetch_assoc())
	{
		$sumat=$sumat + $s['oceny'];
...
$poms=0;
$suma=0;
if ($result891 = $wynik->query("SELECT `oceny` FROM `ocenys2` "))
{
...
if ($result11 = $wynik->query("SELECT ocenys2.oceny from loginy,oceny where loginy.id=oceny.kl and loginy.Klasa='$id' "))
{
...
<?php
if ($result131 = $wynik->query("SELECT * FROM `ocenys2`,loginy where idlogin='$uczenid' and idlekcja='$lekcjaid' and `ocenys2`.idnauczyciel=loginy.id"))
{
	...
	<li id="c<?php echo $s['id'] ?>"style="background-color:
		...
		<ul style=" list-style-type:none;padding:0;margin:0;">
			<li id="d<?php echo $s['id'] ?>" style="width:200px;height:90px;background-color:#BDBDBD;z-index:1;font-size:19px;line-height:1.5em;border:2px white solid;border-radius:10px;text-align: center;margin-top:5px;">		
				<?php  
				echo "Zaco "; echo '"';echo $s['zaco'] ;echo '"'; echo "</br>" ;
...
<div style="width:10px;height:25px;background-color:cornflowerblue;float:left;"id="twoja"></div>
<span style="float:left;padding-left:20px;"> <?php echo $sumat ?>  Twoja średnia </span>
</br>
</br>
<div id="clear"style="clear:both"></div>
<div style="width:10px;height:25px;background-color:lightgreen;float:left;"id="twoja"></div>
<span style="float:left;padding-left:20px;"><?php echo $suma ?>Średnia szkoły</span>
</br>
</br>
<div id="clear"style="clear:both"></div>
<div style="width:10px;height:25px;background-color:orange;float:left;"id="twoja"></div>
<span style="float:left;padding-left:20px;"> <?php echo $sua ?>Średnia klasy</span>
</br>
</br>
<div id="clear"style="clear:both"></div>
<canvas id="mycanvas" width="256" height="256"></div>
