...
<?php
   ...
   $i=0;
    ...
    while($w=$result->fetch_assoc()){
		...
		if ($result1 = $wynik->query("SELECT COUNT(`oceny`) FROM oceny WHERE (`oceny`=5 or `oceny`=6) and `idlogin`='$iducznia'")) {
...
var myChart = new Chart(ctx, {
    ...
    options: {
		title:{
          display:true,
          text:'Ranking uczniow ktorzy maja najwiencej 5 lub 6',
          fontSize:25
        },
        scales: {
            yAxes: [{
