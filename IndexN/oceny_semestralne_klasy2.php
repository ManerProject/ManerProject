...
<form method="post" action="oceny_semestralne_klasy.php">
<input type='submit' style="height:30px"value="Oceny klasy na I semestr">
</form> 
<br>
...
<table border="1">
	...
	<?php
	if($rezultat=$wynik->query("SELECT * FROM `ocenysemestralne` WHERE idklasy='$klasa' AND Semestr=2"))
	{
		$k=$rezultat->num_rows;
		$licz=0;
		for($i=0;$i<$k;$i++)
		...
		while($w=$rezultat->fetch_assoc())
		{
			...
			if($rezult2=$wynik->query("SELECT * FROM `ocenysemestralne` WHERE `iducznia`='$idu' AND Semestr='2'"))
			{
				$l=0;
				while($y=$rezult2->fetch_assoc())
				{
					$lekcja=$y['idlekcja'];
					if($rezult11=$wynik->query("SELECT * FROM `ocenysemestralne` WHERE idlekcja='$lekcja' AND `iducznia`='$idu' AND Semestr='2'"))
					{
...
echo $l."<br>Ilość uczniów nieklasyfikowanych do kolejnej klasy: ";
$l=0; 
for($i=0;$i<$licz;$i++)
{
	if($tab[$i]>2)
